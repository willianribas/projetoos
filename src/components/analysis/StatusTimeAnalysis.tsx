  ...
  return <Card className="border-muted bg-card/50 backdrop-blur-sm hover:shadow-lg transition-all duration-300 animate-fade-in">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <div className="flex gap-2">
          {statuses.map(status => <Button key={status} variant={selectedStatus === status ? "default" : "outline"} size="sm" onClick={() => handleStatusClick(status)} className="transition-all duration-300 hover:scale-105" style={{
      ...
      <CardContent className="space-y-6">
        <div className="h-[300px] animate-scale-in">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={filteredData} className="animate-fade-in">
              ...
              <XAxis dataKey="status" stroke="currentColor" className="text-muted-foreground text-xs" />
              <YAxis label={{
              ...
              <Tooltip formatter={(value: number) => [`${Math.round(value / 24 * 10) / 10} dias (${Math.round(value)} horas)`, "Tempo Médio"]} contentStyle={{
              ...
              {filteredData.map(item => <Bar key={item.status} dataKey="hours" fill={getStatusColor(item.status)} radius={[4, 4, 0, 0]} className="animate-fade-in" />)}
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div className="space-y-4 animate-fade-in">
          ...
          hours,
          days
        }) => <div key={status} className={`space-y-2 p-3 rounded-lg transition-all duration-300 cursor-pointer hover:bg-muted/50 ${selectedStatus === status ? 'ring-2 ring-offset-2' : ''}`} style={{
          borderColor: getStatusColor(status),
          backgroundColor: selectedStatus === status ? `${getStatusColor(status)}10` : undefined
              ...
              <Progress value={hours / maxHours * 100} className="h-2 animate-scale-in" style={{
